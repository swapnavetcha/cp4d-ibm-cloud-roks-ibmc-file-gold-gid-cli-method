...
4)Install oc cli and cpd-cli in your local host from where you will be running the installation commands

5)Get the oc login credentials from the cluster

6)Configure Openshift to deploy CP4D

7)Create a oc project/namespace for deploying cp4d

8)Get the entitlement key for your entitled software library.
   
    i)Log in to Container software library on My IBM with the IBM ID and password that are associated with the entitled software.
